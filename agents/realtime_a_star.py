    ...
    def applyHeuristic(self, initialGame: Game):
        ...
        while not pq.empty():
            ...
            for nextGameState in adjacentStates:
                if (isFirstMove):
                    pq.put((g + self.calculateHeuristic(nextGameState), nextGameState, nextGameState))
                else:
                    pq.put((g + self.calculateHeuristic(nextGameState), nextGameState, firstMoveInPath))
